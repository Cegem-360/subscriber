...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    @foreach ($categories as $category)
        <div @class([
            ...
            <input type="radio" name="{{ $statePath }}" value="{{ $category->id }}"
                wire:model.live="{{ $statePath }}" class="sr-only" id="category-{{ $category->id }}"
            ...
            <div class="mt-6 grow flex items-end">
                <button type="button" wire:click="$set('{{ $statePath }}', {{ $category->id }})"
