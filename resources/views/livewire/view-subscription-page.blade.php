@php
    $hasCustomPrice = $subscription->plan?->hasCustomPriceForTeam($subscription->team) ?? false;
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
            ...
            <div class="flex items-center gap-3">
                @if ($subscription->isActive())
                    <span
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 rounded-full">
                        <span class="w-2 h-2 bg-green-500 rounded-full"></span>
                        {{ __('Active') }}
                    </span>
                @elseif($subscription->ends_at)
                    <span
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 rounded-full">
                        <span class="w-2 h-2 bg-yellow-500 rounded-full"></span>
                        {{ __('Cancelled') }}
                    </span>
                @else
                    <span
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300 rounded-full">
            ...
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <dd class="mt-1 text-gray-900 dark:text-white">
                        {{ $subscription->plan?->planCategory?->name ?? '-' }}</dd>
                </div>
                <div>
                ...
                <div>
                    ...
                    <dd class="mt-1 text-gray-900 dark:text-white">
                        {{ Number::currency((float) $effectivePrice, 'HUF', 'hu', 0) }}/{{ $subscription->plan?->billing_period?->getLabel() }}
                        @if ($hasCustomPrice)
            ...
            <dl class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 text-center">
                    ...
                    <dd class="mt-1 text-3xl font-bold text-blue-600 dark:text-blue-400">
                        {{ $subscription->members->count() + 1 }}</dd>
                </div>
                <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 text-center">
                    ...
                    <dd class="mt-1 text-3xl font-bold text-green-600 dark:text-green-400">
                        {{ $subscription->availableSeats() }}</dd>
                </div>
            </dl>
            ...
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <dd class="mt-1 text-xl font-bold text-gray-900 dark:text-white">
                        {{ Number::currency((float) $effectivePrice * ($subscription->quantity ?? 1), 'HUF', 'hu', 0) }}
                        @if ($hasCustomPrice)
                            <span class="ml-2 text-xs font-normal text-primary-600 dark:text-primary-400">{{ __('(egyedi)') }}</span>
                        @endif
                    </dd>
                </div>
                <div>
                    ...
                    <dd class="mt-1 text-gray-900 dark:text-white">
                        {{ $subscription->created_at?->format('Y. m. d. H:i') }}</dd>
                </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-900 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
                ...
                <dl class="grid grid-cols-1 gap-2 text-sm font-mono">
                    ...
                    <div class="flex gap-2">
                        ...
                        <dd class="text-gray-700 dark:text-gray-300">{{ $subscription->stripe_status?->value ?? '-' }}
                        </dd>
                    </div>
                </dl>
            </div>
